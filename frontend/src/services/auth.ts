...
export async function forgotPassword(email: string): Promise<{ message: string }> {
  ...
  if (!res.ok) {
    throw { message: data.message || 'Failed to send reset email', errors: data.errors };
  }
  return data;
}
...
export async function resetPassword(email: string, token: string, password: string): Promise<{ message: string }> {
  ...
  const data = await res.json();
  if (!res.ok) {
    throw { message: data.message || 'Failed to reset password', errors: data.errors };
  }
  return data;
}
...
export async function registerUser(input: RegisterInput): Promise<RegisterResponse> {
  ...
  const data = await res.json();
  if (!res.ok) {
    throw { message: data.message || "Registration failed", errors: data.errors };
  }
  return data;
}
